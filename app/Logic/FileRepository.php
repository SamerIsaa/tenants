...
class FileRepository {
    ...
    public function upload( $form_data, $type ) {
        $validator = Validator::make($form_data, \App\Models\File::$rules,\App\Models\File::$messages);
        if ($validator->fails()) {
    ...
    public function delete($filename) {
        $sessionImage =  \App\Models\File::where('file_name', 'like', $filename)->first();

        if(empty($sessionImage))
        {
            $sessionImage  =  \App\Models\File::where('display_name', 'like', $filename)->first();
            if(empty($sessionImage)) {
                return Response::json([
                    'error' => true,
                    'file_name' => $filename,
        ...
        if ( \App\Models\File::exists( $path ) ) {
